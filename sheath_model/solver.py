...
class ZhaoSheathSolver:
    ...
    def _type_a_e2_sum_at_infinity(self, phi0_V: float, phi_m_V: float, n_swe_inf_m3: float) -> float:
        ...
        e2_swe_f = (
            (p.T_swe_eV / p.T_phe_eV) * (n_swe_inf_m3 / p.n_phe_ref_m3)
            * (
        ...
        e2_swe_r = (
            2.0 * (p.T_swe_eV / p.T_phe_eV) * (n_swe_inf_m3 / p.n_phe_ref_m3)
            * (
        ...
        e2_swi = (
            2.0 * (p.T_swe_eV / p.T_phe_eV) * (p.n_swi_inf_m3 / p.n_phe_ref_m3)
            * p.mach * p.mach
    ...
    def _try_root_guesses(self, func, guesses: Iterable[np.ndarray]) -> np.ndarray:
        ...
        for guess in guesses:
            ...
            fnorm = float(np.linalg.norm(sol.fun)) if sol.fun is not None else float("inf")
    ...
    def solve_unknowns(self, branch: Branch, guess: tuple[float, ...] | None = None) -> Dict[str, float | str]:
        ...
        if branch == "A":
            ...
            phi0_V, phi_m_V, n_swe_inf_m3 = self._try_root_guesses(self._residuals_type_a, guesses)
        elif branch == "B":
            guesses = [np.array(guess, dtype=float)] if guess is not None else [
                np.array([1.3, 7.0e6]),
                np.array([0.8, 6.5e6]),
            ...
            phi0_V, n_swe_inf_m3 = self._try_root_guesses(self._residuals_type_b, guesses)
            phi_m_V = math.nan
        elif branch == "C":
            guesses = [np.array(guess, dtype=float)] if guess is not None else [
                np.array([-0.5, 6.0e6]),
            ...
            try:
                ...
                if not p.allow_type_c_normal_ion_fallback or p.ion_drift_mode == "normal":
                    raise
                p2 = replace(p, ion_drift_mode="normal", allow_type_c_normal_ion_fallback=False)
        ...
        return {
            ...
            "phi_m_hat": float(phi_m_V / p.T_phe_eV) if math.isfinite(phi_m_V) else math.nan,
    ...
    def _densities_hat(
        ...
        tau = p.tau
        sin_alpha = math.sin(p.alpha_rad)
        n_swi_hat = (p.n_swi_inf_m3 / p.n_phe_ref_m3) * (1.0 - 2.0 * phi_hat / (tau * p.mach * p.mach)) ** (-0.5)

        if branch == "A":
            raise ValueError("Type A requires side-resolved densities; use _densities_hat_type_a_side().")
        elif branch == "B":
    ...
    def _rho_hat_from_densities(dens: Dict[str, np.ndarray]) -> np.ndarray:
        return dens["n_swi_hat"] - dens["n_swe_f_hat"] - dens["n_swe_r_hat"] - dens["n_phe_f_hat"] - dens["n_phe_c_hat"]
    ...
    def _build_type_a_profile(self, uk: Dict[str, float | str]) -> Dict[str, np.ndarray | float | str]:
        ...
        out["n_total_hat"] = out["n_swe_f_hat"] + out["n_swe_r_hat"] + out["n_phe_f_hat"] + out["n_phe_c_hat"]
    ...
    # Public profile API
    # ------------------------------------------------------------------
    def solve_profile(self, branch: Branch, guess_unknowns: tuple[float, ...] | None = None) -> Dict[str, np.ndarray | float | str]:
        p = self.p
        ...
        if uk.get("ion_drift_mode") != p.ion_drift_mode or uk.get("electron_drift_mode") != p.electron_drift_mode:
            p2 = replace(
                p,
        ...
        sol = solve_bvp(rhs, bc, z_hat, np.vstack([phi_guess, e_guess]), tol=1e-4, max_nodes=50000)
        if not sol.success:
    ...
    def solve_auto(self, prefer_stable: bool = True) -> Dict[str, np.ndarray | float | str]:
