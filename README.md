...
Fortran では任意の光電子エネルギースペクトルを入力し、**電位から電場・流束・電流を評価**することもできます。
...
| 試行電位での静的状態 | `evaluate_sheath_state` | 電位・プラズマ・光電子源 → 符号付き E²・流束・電流・接続残差 |
| J=0 の空間分布 | `solve_profile` | 定常解の入力・積分設定 → 高さ・電位・電場・密度 |

単位は **SI、温度と法線運動エネルギーは eV**。初期化は不要で、入力型を渡して結果型とステータスを受け取ります。
...
任意スペクトルの電位掃引は `fpm run --example potential_scan` で実行できます。
源の入力形式と評価結果は [スペクトルと静的評価 API](docs/spectral-api.md) にまとめています。
...
Python は Maxwell 源の J=0 モデルに対応し、任意スペクトルと静的電位評価は Fortran API で利用できます。
...
| [スペクトルと静的評価](docs/spectral-api.md) | 任意 bin 源、A/B/C の電位評価、符号付き E² と接続残差 |
| [運動論モデル](docs/kinetic-model.md) | 軌道保存、半無限上流条件、解の採用条件 |
| [Algorithm notes](docs/algorithm.md) | 共通モデルの式、スペクトル積分、数値精度 |
